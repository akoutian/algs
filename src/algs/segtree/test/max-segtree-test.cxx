...
#include "segtree/max-segtree.hxx"
...
TEST_CASE("Test method `update`: 1 - update one element")
{
    std::vector<int> values(7);
    segtree::max_segtree st(values);
...
TEST_CASE("Test method `update`: 2 - update multiple elements")
{
    std::vector<int> values(7);
    segtree::max_segtree st(values);
...
TEST_CASE("Test method `update`: 3 - large segtree")
{
    std::vector<int> values(1'000'000);
    segtree::max_segtree st(values);

    for (size_t ii{1'000}; ii < 800'000; ++ii)
    {
        st.update(ii, 3);
    }

    for (size_t ii{100}; ii < 500'000; ++ii)
    {
        st.update(ii, 8);
    }
    ...
    for (size_t ii{500'000}; ii < 800'000; ++ii)
    {
        CHECK(st.get(ii) == 3);
    }
}
